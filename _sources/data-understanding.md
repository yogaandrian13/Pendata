...
Metode Random Forest dipilih pada penelitian ini karena sesuai untuk menyelesaikan permasalahan klasifikasi multikelas, yaitu memprediksi GRADE mahasiswa berdasarkan atribut-atribut yang terdapat pada dataset.  proses dimulai dari pembacaan data menggunakan CSV Reader, kemudian dilakukan seleksi atribut melalui Column Filter dan penanganan data yang kosong menggunakan Missing Value agar kualitas data lebih baik. Selanjutnya, kolom GRADE diubah menjadi tipe kategorikal menggunakan Number to String sehingga dapat digunakan sebagai target klasifikasi. Dataset kemudian dibagi menjadi data latih dan data uji menggunakan Table Partitioner. Pada tahap pelatihan, Random Forest Learner membangun banyak pohon keputusan (Decision Tree) dari data latih, kemudian hasil prediksi dari setiap pohon digabungkan menggunakan mekanisme majority voting untuk menentukan kelas akhir. Pendekatan ini membuat Random Forest lebih stabil dan mampu mengurangi risiko overfitting dibandingkan penggunaan satu pohon keputusan saja. Setelah model terbentuk, proses prediksi dilakukan menggunakan Random Forest Predictor, kemudian hasilnya dievaluasi melalui Scorer untuk mengetahui tingkat akurasi model.
## hasil
![alt text](image-46.png)
Berdasarkan hasil evaluasi menggunakan Confusion Matrix, model klasifikasi Random Forest berhasil mengklasifikasikan 10 data dengan benar (Correct Classified) dan 19 data salah diklasifikasikan (Wrong Classified) dari total 29 data uji. Hasil tersebut menghasilkan nilai accuracy sebesar 34,483%, yang menunjukkan bahwa sekitar sepertiga dari seluruh data uji berhasil diprediksi sesuai dengan kelas GRADE yang sebenarnya. Sementara itu, error rate sebesar 65,517% menunjukkan bahwa masih terdapat cukup banyak data yang diprediksi ke kelas yang berbeda. Nilai Cohen's Kappa sebesar 0,203 mengindikasikan bahwa tingkat kesesuaian antara hasil prediksi model dengan data sebenarnya masih tergolong rendah hingga cukup (fair agreement), namun sudah lebih baik dibandingkan prediksi secara acak.